...
*/
// javascript binary operations are only 32-bit, so manipulate as 36-bit binary strings
function makeMask(maskStr) {
  var zeroes = new Set();
  var ones = new Set();
  for (var i = 0; i < maskStr.length; i++) {
    switch (maskStr[i]) {
      case '0': zeroes.add(i); break;
      case '1': ones.add(i); break;
    }
  }
  return {
    apply: function(dec) {
      var valStr = dec.toString(2).padStart(36, '0');
      var result = "";
      for (var i = 0; i < 36; i++) {
        if (zeroes.has(i)) {
          result += '0';
...
function sum(input) {
  ...
  for (const line of input) {
    var [opStr, valStr] = line.split(' = ', 2);
    if (opStr === 'mask') {
      mask = makeMask(valStr);
    } else { // mem[addr]
      var addr = parseInt(opStr.substring(4), 10); // skip 'mem['
      var dec = parseInt(valStr, 10 /* decimal */);
      var maskedStr = mask.apply(dec);
      var maskedVal = parseInt(maskedStr, 2 /* binary */)
      mem.set(addr, maskedVal);
      // console.log(addr, maskedStr, maskedVal);
    }
  }
  var result = 0;
...
console.assert(sum(day14test) === 165);
...
function makeMask2(maskStr) {
  return {
    makeGenerator: function*(dec) {
      var valStr = dec.toString(2).padStart(maskStr.length, '0');
      var stack = []
      var i = 0;
      var prefix = "";
      while (true) {
        if (i === maskStr.length) {
          yield prefix;
          // next, take alternate path using 1
          i = stack.pop();
          if (i === undefined) {
            // no more branches
            return;
          ...
          switch (maskStr[i]) {
            case '0': prefix += valStr[i]; break;
            case '1': prefix += '1'; break;
            case 'X': prefix += '0'; stack.push(i); break; // branch later
          }
        }
        i++;
      } // wend
...
var testMask2 = makeMask2('000000000000000000000000000000X1001X');
var gen42 = testMask2.makeGenerator(42);
console.assert(gen42.next().value === '000000000000000000000000000000011010');
console.assert(gen42.next().value === '000000000000000000000000000000011011');
console.assert(gen42.next().value === '000000000000000000000000000000111010');
console.assert(gen42.next().value === '000000000000000000000000000000111011');
...
function sum2(input) {
  ...
  for (const line of input) {
    var [opStr, valStr] = line.split(' = ', 2);
    if (opStr === 'mask') {
      mask = makeMask2(valStr);
    } else { // mem[addr]
      var addr = parseInt(opStr.substring(4), 10); // skip 'mem['
      var val = parseInt(valStr, 10 /* decimal */);
      for (const astr of mask.makeGenerator(addr)) {
        var adec = parseInt(astr, 2 /* binary */);
        mem.set(adec, val);
        // console.log(astr, adec, val);
      }
    }
  }
  var result = 0;
...
console.assert(sum2(day14test2) === 208);

console.log(sum2(day14input));
