...
Your puzzle input is 598162734.
*/
function makeCrabCups(input) {
  var cups = input.split('').map(c => parseInt(c, 10));
  var cupCount = cups.length;
  var minCup = Math.min(...cups);
  var maxCup = Math.max(...cups);
  return {
    cups,
    showDebug: false,
    moved: 0,
    safeDecrement: function(i) {
      var result = i - 1;
      return (result >= minCup) ? result : maxCup;
    },
    move: function() {
      this.moved++;

      // read current at head and move it to the tail
      var current = this.cups.shift();
      this.cups.push(current);

      // pick up next three; Set preserves insertion order
      var pickup = new Set();
      for (var i = 0; i < 3; i++) { 
        pickup.add(this.cups.shift()); 
      }
      ...
      var destIndex = this.cups.findIndex(e => e === destination);
      if (this.showDebug) {
        console.log('-- move ' + this.moved + ' --');
        console.log('pick up:', Array.from(pickup));
        console.log('destination:', destination, destIndex);
      }
      
      // cups = head_destination + pickup + dest+1_end
      this.cups = this.cups.slice(0, destIndex+1)
        .concat(Array.from(pickup))
        .concat(this.cups.slice(destIndex+1));
        
      return this.cups;
    }, // move()
    orderAfter: function(target) {
      var index = this.cups.findIndex(e => e === target);
      return this.cups.slice(index+1) // immediately after target
        .concat(this.cups.slice(0, index)) // ring before target
        .join(''); 
    },
  };
}
var testCrabCups = makeCrabCups('389125467');
console.assert(testCrabCups.cups.join() === [3,8,9,1,2,5,4,6,7].join());
console.assert(testCrabCups.safeDecrement(1) === 9);
console.assert(testCrabCups.orderAfter(1) === '25467389');
console.assert(testCrabCups.move().join() === [2,8,9,1,5,4,6,7,3].join());
while (testCrabCups.moved < 10) { testCrabCups.move(); }
console.assert(testCrabCups.orderAfter(1) === '92658374');
while (testCrabCups.moved < 100) { testCrabCups.move(); }
console.assert(testCrabCups.orderAfter(1) === '67384529');
