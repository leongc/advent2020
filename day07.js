...
*/
function goldBagColors(input) {
  var goldSet = new Set();
  var containerContentMap = new Map(); // container => Map(color, qty)
  var colorContainerMap = new Map(); // color => Set(containers)
  for (var i = 0; i < input.length; i++) {
    var [container, contents] = input[i].split(' bags contain ', 2);
    if (contents == 'no other bags.') {
      // containerContentMap.set(container, null); // use undefined instead
      // console.log('empty ', container);
      continue;
    }
    var contentMap = new Map();
    contents.replaceAll(/ bags?\.?/g, '').split(', ').forEach( qtyColor => { 
        var [ignore, color] = qtyColor.split(/\d+ /, 2);
        if (colorContainerMap.has(color)) {
          colorContainerMap.get(color).add(container);
        } else {
          colorContainerMap.set(color, new Set().add(container));
        }
        var qty = parseInt(qtyColor);
        contentMap.set(color, qty);
        // console.log(qty, color);
      } );
    if (contentMap.has('shiny gold')) {
      goldSet.add(container);
      // console.log('found gold! ', container);
    } 
    containerContentMap.set(container, contentMap);
    // console.log(container, contentMap);
  }
  
  var c;
  var queue = Array.from(goldSet);
  ...
  while (c = queue.pop()) {
    ...
    evaluated.add(c);
    var containers = colorContainerMap.get(c);
...
var testColors = goldBagColors(day07test);
console.assert(['bright white', 'muted yellow', 'dark orange', 'light red'].every( c => testColors.has(c) ));
console.assert(testColors.size === 4);
...
console.log(goldBagColors(day07input).size);
...
*/
function getContainerContentMap(input) {
  var containerContentMap = new Map(); // container => Map(color, qty)
  for (var i = 0; i < input.length; i++) {
    var [container, contents] = input[i].split(' bags contain ', 2);
    if (contents == 'no other bags.') {
...
function getSize(color, containerContentMap, bagSizes = new Map()) {
  if (bagSizes.has(color)) {
      return bagSizes.get(color);
  }
  var contentMap = containerContentMap.get(color);
  if (contentMap === undefined) {
    return 0;
  }
  var count = Array.from(contentMap.entries()).reduce( (count, [key, value]) => { 
      return count + value + (value * getSize(key, containerContentMap, bagSizes));
  }, 0);
  bagSizes.set(color, count);
  return count;
}
var testCCM = getContainerContentMap(day07test);
console.assert(getSize('dark olive', testCCM) === 7);
console.assert(getSize('shiny gold', testCCM) === 32);
...
console.assert(getSize('shiny gold', getContainerContentMap(day07test2)) === 126);

console.log(getSize('shiny gold', getContainerContentMap(day07input)));
