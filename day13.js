...
function nextBus(input) {
  var i = input[0];
  var buses = input[1].split(',').filter( e => e !== 'x' ).map( s => parseInt(s, 10) );
  while (true) {
    var bus = buses.find( b => i % b === 0 );
    if (bus !== undefined) { 
      return bus * (i - input[0]);
    }
    i++;
  }
}
console.assert(nextBus(day13test) === 295);
...
console.log(nextBus(day13input));
...
console.assert(busSeq([0,'67,7,59,61'])===754018);
console.assert(busSeq([0,'67,7,x,59,61'])===1261476);
console.assert(busSeq([0,'1789,37,47,1889'])===1202161486);

console.log(busSeq(day13input, 100000000000000));
