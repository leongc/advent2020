...
function isValid(inputLine) {
  var [policy, password] = inputLine.split(':', 2);
  var [range, letter] = policy.split(' ');
  var matches = password.match(new RegExp(letter, "g"));
  var count = matches == null ? 0 : matches.length;
  var [low, high] = range.split('-');
  return parseInt(low) <= count && count <= parseInt(high);
}

console.assert(isValid("1-3 a: abcde"));
console.assert(!isValid("1-3 b: cdefg"));
console.assert(isValid("2-9 c: ccccccccc"));
...
console.log(day02input.filter(isValid).length);
...
function isValid2(inputLine) {
  var [policy, password] = inputLine.split(':', 2);
  var [range, letter] = policy.split(' ');
  var [low, high] = range.split('-');
  return !!((password[parseInt(low)] === letter) ^ (password[parseInt(high)] === letter)); // convert binary xor into boolean
}

console.assert(isValid2("1-3 a: abcde"));
console.assert(!isValid2("1-3 b: cdefg"));
console.assert(!isValid2("2-9 c: ccccccccc"));

console.log(day02input.filter(isValid2).length);
